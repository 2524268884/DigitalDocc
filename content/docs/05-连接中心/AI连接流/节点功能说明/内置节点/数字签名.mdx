---
title: "数字签名"
description: "数字签名处理节点，支持多种加密算法生成数字签名。通过配置不同的签名算法、内容编码和密钥，可以实现安全的数据签名和验证功能，适用于API认证、数据完整性校验等场景。"
---
...
<Accordions>
<Accordion title="Q: HMAC和普通哈希算法有什么区别？">
**A:** HMAC算法使用密钥进行签名，提供认证功能；普通哈希算法只提供完整性校验。HMAC算法安全性更高，推荐用于需要认证的场景。
</Accordion>
<Accordion title="Q: 如何选择合适的编码格式？">
**A:** 根据使用场景选择：
- 网络传输：Base64
- 数据库存储：Hex
...
<Accordion title="Q: 签名结果的长度是固定的吗？">
**A:** 取决于算法：
- MD5：128位（32字符Hex）
- SHA-1：160位（40字符Hex）
